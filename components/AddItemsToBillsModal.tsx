...
export function AddItemsToBillsModal() {
  const [open, setOpen] = useState(false);
  const [tagSuggestions, setTagSuggestions] = useState<TagSuggestion[]>([]);
  const [tagEdits, setTagEdits] = useState<
    Record<string, { targetType: StatementTagTargetType; targetSection: "bills_account" | "checking_account" | "spanish_fork" | null; targetName: string }>
  >({});
  ...
  async function loadTagSuggestions() {
    ...
    try {
      ...
      const data = (await res.json()) as { ok?: boolean; items?: TagSuggestion[]; message?: string; subsections?: { bills?: string[]; subscriptions?: string[] }; billNames?: Record<string, string[]> };
      ...
      setTagStatus("success");
      setTagMessage(`Loaded ${items.length} rows. Save in batches of ${BATCH_SIZE}.`);
    } catch (err) {
      setTagStatus("error");
  ...
  async function saveAllInBatches() {
    ...
    while (unsaved.length > 0) {
      ...
      const items = batch.map((t) => {
        const edit = tagEdits[t.id] ?? t.suggestion;
        return { statementId: t.id, targetType: edit.targetType, targetSection: edit.targetSection, targetName: edit.targetName };
      });
      const res = await fetch("/api/statement-tags", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                        ...
                        <div className="flex-shrink-0 flex items-center gap-1 flex-wrap w-full sm:w-auto">
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              setTagEdits((prev) => ({
                                ...prev,
                                [row.id]: { ...edit, targetType: nextType },
                              }));
                            }}
                          >
                            ...
                            <>
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  setTagEdits((prev) => ({
                                    ...prev,
                                    [row.id]: { ...edit, targetSection: nextSection },
                                  }));
                                }}
                              >
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  setTagEdits((prev) => ({
                                    ...prev,
                                    [row.id]: { ...edit, targetName: e.target.value },
                                  }))
                                }
                              >
                                {(() => {
