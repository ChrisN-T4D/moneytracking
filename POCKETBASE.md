...
1. In `.env.local` set:
   - `NEXT_PUBLIC_POCKETBASE_URL` (your PocketBase URL, e.g. `https://pbnmt.c.robpneu.com/_/`)
   - `POCKETBASE_ADMIN_EMAIL` and `POCKETBASE_ADMIN_PASSWORD` (PocketBase admin login)
   - Optionally `POCKETBASE_API_URL` — if you get **404** on setup, your API may be at the root (e.g. `https://pbnmt.c.robpneu.com`); set this to that URL and leave `NEXT_PUBLIC_POCKETBASE_URL` as-is for the app
